class Code_gen:
    ...
    def symbol_table_lookup(self, lexeme):
        for i in range(len(self.symbol_table)):
            if self.symbol_table[len(self.symbol_table)-1-i][0] == lexeme:
                return (len(self.symbol_table)-1-i)
        self.semantic_errors.append("#"+ str(self.parser.scanner.line_number) + ": Semantic Error! " + "\'" + lexeme +"\' is not defined.")
        return 0
    ...
    def write_method(self,method_name):
        if method_name == "#j_main":
            ...
            self.semantic_stack.append(self.parser.token)
            ...
            if var_type == "void":
                self.semantic_errors.append("#"+ str(self.parser.scanner.line_number) + ": Semantic Error! Illegal type of void for \'" + var_lexeme + "\'.")
            var_scope = self.scope
            self.symbol_table_insert(var_lexeme , "var" , 1 , var_type , var_scope)

        elif method_name == "#dec_array":
            array_size = int(self.parser.token)
            ...
            while True:
                try:
                    ...
                    self.semantic_errors.append("#"+ str(self.parser.scanner.line_number) + ": Semantic Error! No 'repeat ... until' found for 'break'.")
                    i = 1
                    break
            ...
            if self.parser.token == "output":
                self.semantic_stack.append(self.parser.token)
            else:
                # push data address of variable in semantic stack
                var_data = self.symbol_table[self.symbol_table_lookup(self.parser.token)]
            ...
            self.program.append(
                "(ASSIGN, " + "#" + str(self.parser.token) + ", " + str(temp_address) + ",   )")
            self.program_line += 1
        ...
        #         "(ASSIGN, " + str(value_address) + ", @" + str(indirect_address) + ",   )")
            ...
            if function_address == "output":
                ...
                if len(self.current_called_method_params) != function_params_number:
                    self.semantic_errors.append("#"+ str(self.parser.scanner.line_number) 
